...
This is because the light elements - Hydrogen and Helium - are either fully ionized (\ce{H+} and \ce{He++}) so they have no electrons, or have absorption lines in the far UV where the flux is low (\ce{He+}).
...
\todo[inline]{Oscillation strength is defined as the ratio of the quantum to the classical transition probability.
It is not really clear how the number \num{.01} is found, but the idea is that only permitted transitions contribute, which makes sense.}
