...
\todo[inline]{Scegli come uniformare le date delle lezioni, ossia se metterle o no innanzitutto, e dove}
...
\end{proposition}
\todo[inline]{Magari due righe su cosa significa omogeneo ed isotropo}
...
\todo[inline]{Suppose that absolute curvature k is a real number, than it is clear that for each value of $k>0$ it exists a change of cohordinates that we can rewrite the metric with $k'=1$ and similarly for $k<0$. In this sense if we suppose that each value of k has the same probability to be that one of our universe, the $k=0$ value has 0 measure}
