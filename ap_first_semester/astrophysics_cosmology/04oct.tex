...
Numerically, inserting reasonable estimates for the parameters, we get \(\mathscr{L}_{g} \approx \num{2.0(7)e8} h L_\odot \SI{}{\per\cubic\mega\parsec}. \) 
...
It is not the possible one: in principle dark energy could be constituted by any kind of fluid which is uniformly distributed and which has negative pressure. 
...
So, with the data we have currently we cannot determine the sign of the universe's spatial curvature: with the current experimental uncertainties, the value is very much compatible with \(\Omega_{k} =0 \), and no particular meaning should be drawn from the fact that the nominal value of \(\Omega_{k}\) is slightly above 0.
