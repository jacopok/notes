...
\end{equation}
%
but do note that \(R\) is a constant: given \((x^{0}, x^{1}) = (\theta, \varphi)\), we have as before that the only nontrivial derivative is \(g_{11,0} = R^2 2 \sin(\theta ) \cos(\theta) \).

Then this case is exactly analogous to the previous one: we can skip almost all of the computation and jump straight to: 
%
\begin{subequations}
\begin{align}
  \Gamma^{0}_{11} &= -\frac{1}{2} g^{0}g_{11,0} = - \sin(\theta ) \cos(\theta ) \\
