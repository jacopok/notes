...
The starting separation between geodesics is not, properly speaking, a \emph{vector} since it does not belong to the tangent space, however we can be sloppy and approximate it as a tangent vector. So, we take it to be a basis vector \(\xi^{ \mu } = (e_{\widetilde{\alpha} })^{\mu }\): then it is a fact from differential geometry that under our assumptions 
%
\begin{align}
  \xi^{\mu } \nabla_{\mu } u^{\nu } = u^{\mu } \nabla_{\mu } \xi^{\nu }
\,,
