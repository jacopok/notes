...
\begin{align}
  x =d \pm \sqrt{d^2 -2}
  = d \qty(1 \pm \sqrt{1 - \qty(\frac{2}{d})^2})
\,,
\end{align}
%
...
%
\begin{align}
  x = d \qty(1 - \qty(1 - \frac{1}{2} \qty(\frac{2}{d})^2)) = \frac{d}{2}  \qty(\frac{2}{d})^2 = \frac{2}{d} = \frac{4GM}{b} = \sin \varphi 
\,. 
\end{align}
%

\todo[inline]{Is this right? We should find \(2GM/b\), I'd think\dots}
...
\subsubsection{A proof for the geodesic deviation formula}

The starting separation between geodesics is not, properly speaking, a \emph{vector} since it does not belong to the tangent space, however we can be sloppy and approximate it as a tangent vector. So, we take it to be a basis vector \(\xi^{ \mu } = (e_{\widetilde{\alpha} })^{\mu }\): then it is a fact from differential geometry that under an assumption of vanishing torsion (which we always make anyways in GR) and assuming that the vector fields \(u^{\mu }\) and \(\xi^{\mu }\) commute the following holds: 
%
