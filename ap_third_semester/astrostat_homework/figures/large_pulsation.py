...
plt.semilogx(omegas, c, lw=.5, label='$c/N$')
plt.semilogx(omegas, h, lw=.5, label='$h/N$')
plt.semilogx(omegas, s, lw=.5, label='$s/N$')
plt.xlabel('$\\omega$')
plt.grid('on')
# plt.axvline(1, label='$\\Delta^{-1}$', c='black', ls=':')
plt.axvline(2 * np.pi / 2, label='$\\pi \\Delta^{-1}$ = Nyquist pulsation', c='black', ls=':')
