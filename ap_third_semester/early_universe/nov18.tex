...
\end{align}
%
so the Liouville operator can be written as 
%
\begin{align}
...
%
\begin{align}
\frac{4 \pi g}{(2 \pi )^3} \int \dd{E} p^3 \pdv{f}{p} = -3 n(t)
\,.
\end{align}
...
The terms \(1 \pm f_i\) are given by the Fermi enhancement (\(-\)) or Bose blocking (\(+\)) coming from the statistics of the chosen particle.
...
Because of time reversal symmetry we will have \(\abs{\mathcal{M}}^2_{1 + 2 \to 3 + 4} = \abs{\mathcal{M}}^2_{3 + 4 \to 1 +2} \equiv \abs{\mathcal{M}}^2\). 

We can further postulate that all the particle species are in kinetic equilibrium: then, we say that they are distributed according to the Bose-Einstein or Fermi-Dirac distribution, 
%
\begin{align}
...
Further, we make the semiclassical approximation \(1 \pm f_i \approx 1\). 
...
\qty(\exp(\frac{\mu_3 + m_4 }{T}) - \exp( \frac{m_1 + m_2 }{T}))
\end{split}
\,.
\end{align}
