...
We have no source term --- a 0 on the right-hand side --- since we are working up to linear order, in full generality there will be other source terms, commonly denoted as \(\pi^{T}_{ij}\) (\(T\) for ``source of tensor modes'').
...
The polarization tensors are required to be symmetric, traceless, orthogonal to \(\vec{k}\), and to be orthonormal with respect to the product \((\epsilon_1, \epsilon_2 ) = \Tr(\epsilon_1 \epsilon_2^{*}) \). If \(\vec{k} \propto \hat{z}\), then they are given by the usual expressions 
%
\begin{align}
\epsilon_{ij}^{+} = \left[\begin{array}{ccc}
1 & 0 & 0 \\ 
0 & -1 & 0 \\ 
0 & 0 & 0
\end{array}\right] &&
\epsilon_{ij}^{\times } = \left[\begin{array}{ccc}
0 & 1 & 0 \\ 
1 & 0 & 0 \\ 
0 & 0 & 0
\end{array}\right] 
...
%
\begin{align} \label{eq:primordial-gw-power-spectral-amplitude}
